...
StatusOr<string> Exec(const string& cmd, size_t expectedLength) {
  auto process =
      subprocess::Popen(cmd, subprocess::shell{true}, subprocess::output{subprocess::PIPE},
...
StatusOr<string> Exec(const string& cmd, size_t expectedLength, const string& input) {
  auto process =
...
StatusOr<string> Sha256(const string& text) { return Exec("sha256sum | cut -f1 -d' '", 65, text); }
